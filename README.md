...
**性能**：解密核心没有逐字节跑 Python 循环，而是把两个周期序列
（`own_key` 周期 17、`PUB_KEY_MEND` 周期 272）展开成定长数据，用
**大整数异或 + `bytes.translate` 查表**在 C 层批量完成 —— 相比逐字节写法快
**7~17 倍**（40 MB 无损单曲从约 8.6 秒降到约 1.2 秒），且**不依赖 NumPy**。
